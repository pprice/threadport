...
async function readSnapshot(page: Page): Promise<Snapshot> {
  return page.evaluate(() => {
    ...
    const tailMetric =
      document.querySelector('[data-testid="metric-tail"]')?.textContent?.trim() ??
      null
    ...
    return {
      ...
      jumpExists: Boolean(document.querySelector('[data-testid="jump-to-bottom"]')),
...
async function waitForReady(page: Page) {
  ...
  await expect.poll(() => page.locator('.chatVirtualRow').count()).toBeGreaterThan(0)
}
...
async function readPromptGeometry(
  ...
  return page.evaluate((promptText) => {
    ...
    const headInset = Number.parseFloat(
      styles?.getPropertyValue('--chat-head-inset') ?? '0',
    )
  ...
  return page.evaluate(() => {
    ...
    const candidate =
      [...document.querySelectorAll<HTMLElement>('[data-message-id]')]
        ...
        .filter(({ id, rect }) => {
          return Boolean(id) && rect.bottom > viewportRect.top && rect.top < viewportRect.bottom
        })
...
test('jump-to-bottom behavior is stable in every playground shell', async ({
  page,
}) => {
...
test('submitted prompts align below the head inset without moving the composer', async ({
  ...
  for (const example of ['chatgpt', 'mobile'] as const) {
    ...
    expect(geometry.offsetFromHead ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(
      geometry.headInset + 44,
    )
...
test('long streaming content burns through the dynamic tail reserve', async ({
  ...
  await expect(page.getByText('End of the extended long response.')).toBeVisible({
    timeout: 8_000,
  })
  await expect
    .poll(async () => {
...
test('scroll-to-head accounts for the mobile head inset', async ({ page }) => {
  ...
  await expect
    .poll(async () => (await readTopGeometry(page)).scrollTop ?? Number.POSITIVE_INFINITY)
...
test('history prepends preserve the visible anchor while rows stay virtualized', async ({
  ...
  const totalBefore = Number(await page.getByTestId('metric-items').textContent())
  ...
  await expect(page.getByTestId('metric-items')).toHaveText(String(totalBefore + 18))

  await expect
    .poll(async () => {
      const anchorAfter = await readMessageTopById(page, anchorBefore?.id ?? '')

      if (!anchorAfter.exists || anchorAfter.top === null || anchorBefore === null) {
        return Number.POSITIVE_INFINITY
      }
