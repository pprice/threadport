...
async function readItemGeometry(
  page: Page,
  id: string,
): Promise<ItemGeometry> {
...
async function readTailReserveGeometry(
  ...
  return page.evaluate(() => {
    ...
    const minHeight = Number.parseFloat(window.getComputedStyle(reserve).minHeight)
...
test('prop-only inset and threshold changes update state and geometry', async ({
  ...
  expect(compactHead.offsetFromViewportHead ?? Number.POSITIVE_INFINITY).toBeLessThanOrEqual(
    34,
  )
