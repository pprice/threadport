...
const CONTROL_SCROLL_ANIMATION = ThreadPort.Animation.easeOutCubic(420)
const MESSAGE_IN_ANIMATION = ThreadPort.Animation.easeOutQuart(560)
const TAIL_FOLLOW_ANIMATION = ThreadPort.Animation.easeOutCubic(360)
...
const DemoMessageView = memo(function DemoMessageView({
  ...
  return (
    ...
    >
      {!isUser && (
        <div className="avatar">{message.role === 'system' ? 'S' : 'A'}</div>
      )}
  ...
  if (event.type === 'human-message') {
    viewport.scrollToItem(event.id, {
      align: 'head',
      animation: ThreadPort.Animation.easeOutQuart(560),
    })
  }
}`}</code>
          </pre>
        ...
        {message.variant === 'visual' && (
          <div
            ...
            role="img"
            aria-label="Example visual response"
          >
            <span />
            <span />
            <span />
...
export function App() {
  ...
  const [viewportState, setViewportState] =
    useState<ThreadPort.ViewportState | null>(null)
  const [pendingScrollKey, setPendingScrollKey] =
    useState<ThreadPort.ItemKey | null>(null)
  ...
  const activeExample =
    examples.find((item) => item.id === example) ?? examples[0]
  ...
  return (
    <main className="appShell">
      <aside className="controlPanel" aria-label="Examples and controls">
        ...
        <section className="exampleList" aria-label="Example picker">
          {examples.map((item) => (
            <button
              key={item.id}
              className={
                item.id === example ? 'exampleButton active' : 'exampleButton'
              }
        ...
        <div className="controls">
          ...
          <button
            data-testid="add-visual-answer"
            type="button"
            onClick={addVisualAnswer}
          >
            Add visual answer
          </button>
          <button
        ...
        <dl className="metrics">
          ...
          <div>
            <dt>head reserve</dt>
            <dd data-testid="metric-head-reserve">
              {Math.round(headReserve)}px
            </dd>
          </div>
          <div>
            <dt>tail reserve</dt>
  ...
  <ThreadPort.Overlay placement="tail">
    <JumpToBottom />
      ...
      >
        ...
        <ThreadPort.Root className="chatSurface">
          {example === 'mobile' && (
            <div
          ...
          <ThreadPort.Overlay className="jumpDock" placement="tail">
            {showJumpToBottom && (
              <button
